...
async function uploadAll() {
  ...
  if (!SETTINGS.endpoint) { openSettings(); toast('Set up the image upload link first.'); return; }
  ...
  for (const p of PHOTOS) {
    ...
    try {
      ...
      if (!/^https?:\/\//.test(url)) throw new Error(url || 'bad response');
      UPLOADED.set(p.id, url.replace(/^http:/, 'https:'));
      done++;
    } catch (e) { fail++; }
  }
  btn.disabled = false;
  const total = PHOTOS.length, have = UPLOADED.size;
  if (fail) setStatus($('#upload-status'), '⚠ ' + have + '/' + total + ' uploaded, ' + fail + ' failed. Check the upload link in Settings, then tap again.', 'err');
  else setStatus($('#upload-status'), '✓ All ' + total + ' photos uploaded.', 'ok');
}
...
const HELP_HTML =
  '<b>One-time setup (~5 min, free).</b> Phones can’t upload to the photo host directly, so you run a tiny free “relay” that does it for you.' +
  '<ol>' +
