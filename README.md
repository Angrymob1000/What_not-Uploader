...
~3 days, which is plenty: Whatnot copies the images into its own storage when you import, so
the link only has to be alive at import time. Just do the Whatnot import within a few days.

**Optional — permanent links that never expire.** If you'd rather images be hosted forever,
run your own tiny free relay and paste its URL into **⚙ Settings → Permanent image hosting**:
...
4. Copy the worker URL (`https://something.workers.dev`) → paste it in Settings.
